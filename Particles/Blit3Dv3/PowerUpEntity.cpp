...
void PowerUpEntity::Update(float seconds)
{
	...
	while(frameTimer >= timeBetweenFrames)
	{
		...
		currentFrameNum++;
		if(currentFrameNum > 4) //loop the animation frames
			currentFrameNum = 0;
...
PowerUpEntity * MakePowerUp(PowerUpType type, float xpos, float ypos)
{
	...
	switch(type)
	{
		//FASTBALL is a powerDown so it shows different sprites when spawning
	case PowerUpType::FASTBALL:										
		for (int i = 0; i < 5; ++i)
			powerUpEntity->spriteList[i] = fastBallSprites[i];
