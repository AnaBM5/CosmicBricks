...
BallEntity * MakeBall(Sprite *sprite, Sprite * fireballSprite)	//Now receives both sprites
{
	//make a ball entity
	...
	BallBodyDef.angularDamping = 1.2f;
	...
	dynamicBall.m_radius = 20.f / (2 * PTM_RATIO); //ball has diameter of 20 pixels
	...
	//Add a sensor fixture to the ball
	//Defines a circle shape with a bigger diameter than the other fixture
	dynamicBall.m_radius = 60.f / (2 * PTM_RATIO);

	// Define the dynamic body fixture.
	fixtureDef.shape = &dynamicBall;

	//Makes it a sensor so it detects collisions but doesn't interact with the physics
	...
	ballEntity->fireSprite = fireballSprite;
...
{
	//BallEntity has its own draw to change the sprite of the ball depeding on wheter the
	//fireball powerUp is active or not
	...
	if (fireBallTimer > 0)
	{
		fireSprite->angle = rad2deg(body->GetAngle());

		// Draw the objects
		fireSprite->Blit(position.x, position.y);
	}
	else
	{
